...
from ..model import backbones_2d, backbones_3d, dense_heads, roi_heads
from ..model.backbones_2d import map_to_bev
from ..model.backbones_3d import pfe, vfe
from ..model.model_utils import model_nms_utils, network_utils
from ..model import img_backbones
...
class Detector3DTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, dataset, logger):
        ...
        self.module_topology = [
            'img_backbone', 'vfe', 'backbone_3d', 'map_to_bev_module', 'pfe',
    ...
    def build_img_backbone(self, model_info_dict):
        if self.model_cfg.get('IMG_BACKBONE', None) is None:
            return None, model_info_dict
        img_backbone_module = img_backbones.__all__[self.model_cfg.IMG_BACKBONE.NAME](
            model_cfg=self.model_cfg.IMG_BACKBONE
        )
        model_info_dict['module_list'].append(img_backbone_module)
        return img_backbone_module, model_info_dict
    ...
    def load_params_from_file(self, filename, logger, to_cpu=False):
        ...
        loc_type = torch.device('cpu') if to_cpu else torch.device('cuda:0')
